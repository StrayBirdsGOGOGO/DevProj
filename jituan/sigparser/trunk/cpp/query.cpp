/*
 * $Id: query.cpp,v 1.1 2005/04/04 02:45:11 bob Exp $
 */
...
bool Query::prepare(void) {
  ...
  _errno = OCIStmtPrepare(_stmt, _err, (unsigned char *) SQL.c_str(),
      (ub4) SQL.length(), OCI_NTV_SYNTAX, OCI_DEFAULT);
  if (!ORAPP_SUCCESS(_errno)) {
  ...
  _errno = OCIAttrGet(_stmt, OCI_HTYPE_STMT, &_type, &_typelen,
...
bool Query::execute(void) {
  ...
  _errno = OCIStmtExecute(_svc, _stmt, _err, (ub4)(_type != OCI_STMT_SELECT),
      0, NULL, NULL, _execmode);
  ...
  if (_errno != OCI_SUCCESS && _errno != OCI_SUCCESS_WITH_INFO) {
    ...
    if (!(_errno == OCI_NO_DATA && (_type == OCI_STMT_BEGIN || _type
        == OCI_STMT_DECLARE))) {
  ...
  _errno = OCIAttrGet(_stmt, OCI_HTYPE_STMT, &_rows, &rows_size,
      OCI_ATTR_ROW_COUNT, _err);
  if (!ORAPP_SUCCESS(_errno)) {
...
bool Query::bind(const char *name, void *addr, unsigned width, ub2 type) {
  ...
  _errno = OCIBindByName(_stmt, &orabind, _err, (unsigned char *) name, (sb4)
      - 1, addr, (sb4) width, type, NULL, NULL, NULL, 0, NULL,
      OCI_DEFAULT);
  if (!ORAPP_SUCCESS(_errno)) {
