...
int G::loadConfig() {
  ...
  /*
  itr = m_props.find("SYSTEM_LOGIN_ID");
  if(itr != m_props.end()){
    char* pValue = (char*)itr->second.c_str();
    char loginID[LOGINID_LEN];
    int ret = formatString(pValue,LOGINID_LEN,loginID);
    if(-1 == ret){
        LOG4CXX_ERROR(rootLogger, "wrong SYSTEM_LOGIN_ID = "<<pValue);
        return -1;
    }else{
        ZxComm::m_sysID = loginID;
    }
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find SYSTEM_LOGIN_ID in app.cfg !");
    return -1;
  }

  itr = m_props.find("SYSTEM_PWD");
  if(itr != m_props.end()){
    ZxComm::m_sysPwd = itr->second;
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find SYSTEM_PWD in app.cfg !");
    return -1;
  }*/

  itr = m_props.find("SUBSCRIBE_LOGIN_ID");
  if(itr != m_props.end()){
      char* pValue = (char*)itr->second.c_str();
      if(strlen(pValue)>LOGINID_LEN){
          LOG4CXX_ERROR(rootLogger, "wrong SUBSCRIBE_LOGIN_ID = "<<pValue);
          return -1;
      }else{
          ZxComm::m_subscrbLoginID = pValue;
      }
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find SUBSCRIBE_LOGIN_ID in app.cfg !");
    return -1;
  }

  itr = m_props.find("SUBSCRIBE_PWD");
  if(itr != m_props.end()){
    ZxComm::m_subscrbPwd = itr->second;
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find SUBSCRIBE_PWD in app.cfg !");
    return -1;
  }

  itr = m_props.find("PROTOCOL_VERSION");
  if(itr != m_props.end()){
    ZxComm::m_version = atoi(itr->second.c_str());
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find PROTOCOL_VERSION in app.cfg !");
    return -1;
  }
  
  itr = m_props.find("PROTOCOL_SUBVERSION");
  if(itr != m_props.end()){
    ZxComm::m_subVersion = atoi(itr->second.c_str());
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find PROTOCOL_SUBVERSION in app.cfg !");
    return -1;
  }

  itr = m_props.find("CDR_ORDER_ID");
  if(itr != m_props.end()){
    ZxComm::m_CDROrderID = atoll(itr->second.c_str());
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find CDR_ORDER_ID in app.cfg !");
    return -1;
  }
  ...
  itr = m_props.find("BEGIN_END_INTERVAL");
  if(itr != m_props.end()){
    ZxComm::m_beginEndInterval = atoi(itr->second.c_str());
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find BEGIN_END_INTERVAL in app.cfg !");
    return -1;
  }
  
  
  //for sjqm idpparser_test:192.168.1.162/163----------------------------
  itr = m_props.find("TRANSMIT_PORT");
  if(itr != m_props.end()){
    Transmit::m_nPort = atoi(itr->second.c_str());
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find TRANSMIT_PORT in app.cfg !");
    return -1;
  }

  itr = m_props.find("TRANSMIT_HOST_IP");
  if(itr != m_props.end()){
    Transmit::m_sHostIP = itr->second;
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find TRANSMIT_HOST_IP in app.cfg !");
    return -1;
  }  
  
  itr = m_props.find("TRANSMIT_BACKUP_IP");
  if(itr != m_props.end()){
    Transmit::m_sBackupIP = itr->second;
  }else{
    LOG4CXX_ERROR(rootLogger, "Can't find TRANSMIT_BACKUP_IP in app.cfg !");
    return -1;
  }  
  //------------------------------------------------------------------
