...
void CDRMessage::setMessageBuf(PUCHAR pData, UINT dataLen){
    memset(m_CDRBuf,0,CDR_DATA_MAX_LEN);
...
int CDRMessage::parseMessage(){
    ...
    //get CDRLen(2B)
   // m_CDRLen = (UINT)(*ptr<< 8) + *(ptr+1);
    ptr +=2;
    
    //get CDRVersion(1B)
    m_CDRVersion = (UINT)*ptr;
    ptr +=1;
    
    //get VariableIENumber(2B)
    m_VarIENumber = (UINT)(*ptr<< 8) + *(ptr+1);
    ptr +=2;
    
    //get protocol type(2B)
    m_protocolType = (UINT)(*ptr<< 8) + *(ptr+1);
    ptr +=2;
    
    //get CDRID(8B)
    m_CDRID = (ULONGLONG)*ptr;
    for(int i=1;i<8;i++){
        m_CDRID = m_CDRID<<8;
        m_CDRID += (ULONGLONG)*(ptr+i);
    }
    ptr +=8;

    //parse Fixed Load
    if(BICC == m_protocolType){
        ret = parseBICC_Message(ptr);
        if(0 != ret){
            return -1;
        }
    }else if(ISUP == m_protocolType){
        ret = parseISUP_Message(ptr);
        if(0 != ret){
            return -1;
        }
    }else{
        LoggerPtr rootLogger = Logger::getRootLogger();  
        LOG4CXX_ERROR(rootLogger, "CDRReqMessage: wrong protocol Type=0x"<<hex<<m_protocolType<<dec);
        G::displayHex("CDRReqMessage: wrong protocol Type",m_CDRBuf,m_CDRLen);
        return -1;
    }    
...
int CDRMessage::checkDelay(){
    ...
    //check delay
    //startTime = the beginning of a call,  endTime = create DR time
    if(IAM == m_status){
        m_pZxClient->incIAMDRs();
        
        innerDelay = m_endTime[0] - m_startTime[0];
        outerDelay = m_recvTime - m_endTime[0];
        if(innerDelay >= ZxComm::m_msgDelayLimit || outerDelay >= ZxComm::m_msgDelayLimit){
            m_pZxClient->setMaxInnerDelay(innerDelay);
         ...
         /*   if((++nDelay % 10000) == 0){
            ...
            int parserDelay = now - m_endTime[0];
            if( parserDelay >= ZxComm::m_msgDelayLimit){
                if((++nDelay % 1000) == 0){
                    LoggerPtr rootLogger = Logger::getRootLogger();  
                    LOG4CXX_WARN(rootLogger, "!!!parser delay="<<parserDelay <<"s, startTime="<<m_startTime[0]
                    <<", endTime="<<m_endTime[0]<<", m_recvTime="<<m_recvTime<<", now="<<now<<", type="<<m_protocolType);
                    //G::displayHex("delay package ",m_CDRBuf,m_CDRLen);
                }
            }
        }
    }else if(Closure == m_status){
        m_pZxClient->incClosureDRs();
        ...
        if(outerDelay >= ZxComm::m_msgDelayLimit){
         ...
         /*   if((++nDelay % 10000) == 0){
                LoggerPtr rootLogger = Logger::getRootLogger();  
                LOG4CXX_WARN(rootLogger, "outerDelay="<<outerDelay<<"s, endTime="<<m_endTime[0]<<", m_recvTime="<<m_recvTime<<", now="<<now<<", type="<<m_protocolType);
                //G::displayHex("delay package ",rawMsg,bufLen);
            }*/
            return -1;
        }else{
            int parserDelay = now - m_endTime[0];
            if( parserDelay >= ZxComm::m_msgDelayLimit){
                if((++nDelay % 1000) == 0){
                    LoggerPtr rootLogger = Logger::getRootLogger();  
                    LOG4CXX_WARN(rootLogger, "!!!parser delay="<<parserDelay <<"s, endTime="<<m_endTime[0]
                        <<", m_recvTime="<<m_recvTime<<", now="<<now<<", type="<<m_protocolType);
                    //G::displayHex("delay package ",m_CDRBuf,m_CDRLen);
                }
            }
        }
        ...
        LoggerPtr rootLogger = Logger::getRootLogger();  
        LOG4CXX_ERROR(rootLogger, "wrong status="<<m_status<<", type="<<m_protocolType);
        G::displayHex("wrong status",m_CDRBuf,m_CDRLen);
        return -1;
    }
...
//seqNum: CIC_OPCDPC_startTime/sec_startTime/nSec
//length: 22B~49B
int CDRMessage::setSeqNum(){
    int len = 0;
    char ss[40] = {0};

    sprintf(ss, "%lu_%s%s_%lu_%lu",m_CIC,m_opc,m_dpc,m_startTime[0],m_startTime[1]);  
    len = strlen(ss);
    if(len > MAX_SEQ_NUM_LEN){
        LoggerPtr rootLogger = Logger::getRootLogger();  
        LOG4CXX_ERROR(rootLogger, "Too big seqNum: m_CIC="<<m_CIC<<", m_opc="<<m_opc<<", m_startTime="<<m_startTime[0]<<"s"<<m_startTime[1]<<"ns");
        G::displayHex("",m_CDRBuf,m_CDRLen);
        return -1;
    }
    
    memset(m_seqNum, 0, MAX_SEQ_NUM_LEN+1);
    memcpy(m_seqNum, ss, len);
    m_seqNum[len] = 0;
...
int CDRMessage::setMatchedCall(CDRMessage* pMessage){    
    ...
    if(m_protocolType != pMessage->getProtocolType()){
        ...
        LOG4CXX_ERROR(rootLogger, "setMatchedCall: different type(IAM type="<<pMessage->getProtocolType()<<", Closure type="<<m_protocolType
            <<"), IAM("<<matchedCaller<<"->"<<matchedCaller<<"), seqNum="<<m_seqNum);
        return -1;
    }
     ...
     if((0 == strcmp((char*)m_caller, matchedCaller)) && (0 == strcmp((char*)m_called, matchedCalled)))
     {
         ...
         LoggerPtr rootLogger = Logger::getRootLogger();
         LOG4CXX_ERROR(rootLogger, "setMatchedCall: different call, but same key="<<m_seqNum<<", diffEnd="<<diffEnd
     ...
     /*   int callerlen = strlen(matchedCaller);
        ...
        if(matchedCaller && matchedCalled){
            ...
            if(checkCallNumber(m_caller) || checkCallNumber(m_called)){
                LoggerPtr rootLogger = Logger::getRootLogger(); 
                LOG4CXX_ERROR(rootLogger, "setMatchedCall: type=ISUP, get invalid caller="<<m_caller<<", called="<<m_called<<", seqNum="<<m_seqNum);
                return -1;
            }else{
