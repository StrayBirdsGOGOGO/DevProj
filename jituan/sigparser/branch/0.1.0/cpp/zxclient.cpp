  ...
  while (!G::m_bSysStop  && rDataControl.isActive()) {
      ...
      if (nready > 0)
      {
          if (FD_ISSET(fd, &fds))
          {
              ...
              if( 1 > readNum) {
                 LOG4CXX_ERROR(rootLogger, "Receive msg from ZX("<<hostIP<<") failed, nread= "<<readNum<<", errno="<<errno<<", error:" << strerror(errno));
                 pClient->beforeDel();
                 break;
              }else {
  ...
  while (!G::m_bSysStop && rDataControl.isActive()) {
    ...
    if (rDataControl.isActive()) {
        ...
        if(1 == rc){
            ...
            if(pMessage){
                pMessage->setRecvTime();
                pMessage->setPackID(packID);
                pMessage->parseMessageHeader();
...
void ZxClient::checkRecvMsg(){
    ...
    long l = m_nClosure - m_nLastClosure;
    m_nLastClosure = m_nClosure;

    long now = 0;
    time(&now);
    if(i>0){
        LOG4CXX_DEBUG(G::logger, "From "<<m_zxHostIP<<": total="<<i<<"/s(IAM="<<k<<"/s, Closure="<<l<<"/s), now="<<now);
       /* if(j>0){
