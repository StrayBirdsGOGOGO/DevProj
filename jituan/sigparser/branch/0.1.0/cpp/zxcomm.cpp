    ...
    if (pMessage) { 
        ...
        if(1 == ret){
            ...
            if(IAM == status){
                //send now
                bSend = true;
            }else if(Closure == status){
                ret = ZxComm::findMatchedIAM(pMessage);
                if(1 == ret){
                    //matched
                    bSend = true;
                }else if(0 == ret){
                    //dismatched
                    ret = ZxComm::insert2ClosureList(pMessage);
                    if(-1 == ret){
                        //insert failed
                        LoggerPtr rootLogger = Logger::getRootLogger();
                        LOG4CXX_ERROR(rootLogger, "insert2ClosureList failed: type="<<pMessage->getProtocolType()<<", status="<<pMessage->getStatus()<<", seqNum="<<pMessage->getSeqNum());
                        delete pMessage;
                        pMessage = 0;
                    }
                    continue;
                ...
                if(BICC == pMessage->getProtocolType()){
                    LoggerPtr rootLogger = Logger::getRootLogger(); 
                    LOG4CXX_ERROR(rootLogger, "wrong status="<<status<<", type=BICC, seqNum="<<pMessage->getSeqNum()
                        <<", "<<pMessage->getCaller()<<"->"<<pMessage->getCalled());
                }else{
                    LoggerPtr rootLogger = Logger::getRootLogger(); 
                    LOG4CXX_ERROR(rootLogger, "wrong status="<<status<<", type=ISUP, seqNum="<<pMessage->getSeqNum());
                }
                delete pMessage;
                pMessage = 0;
            }
        ...
        if(bSend){
            ...
            if(-1 == ret){
                ...
                LOG4CXX_ERROR(rootLogger, "insertCDR2Map failed: "<<pMessage->getCaller()<<"->"<<pMessage->getCalled()
                    <<", type="<<pMessage->getProtocolType()<<", status="<<pMessage->getStatus()<<", seqNum="<<pMessage->getSeqNum());
                delete pMessage;
                pMessage = 0;
            }else{
                //send 
...
int ZxComm::insert2CDRMap(CDRMessage* pMessage){ 
    ...
    if (iter != m_cdrMap.end())
    {
        ...
        if(oldMessage)
        {
            ...
            LOG4CXX_ERROR(rootLogger, ++m_nRepeatDRs<<": same key="<<key<<", diff="<<diffTime
                <<", old(packID="<<oldMessage->getPackID()<<", type="<<oldMessage->getProtocolType()<<", id="<<oldMessage->getCDRID()<<", "<<oldMessage->getCaller()<<"->"<<oldMessage->getCalled()
                <<"), new(packID="<<pMessage->getPackID()<<", type="<<pMessage->getProtocolType()<<", id="<<pMessage->getCDRID()<<", "<<caller<<"->"<<called<<")");
            delete oldMessage;
            oldMessage = 0;
        }
    } 
    ...
    {
        LoggerPtr rootLogger = Logger::getRootLogger();
        LOG4CXX_DEBUG(rootLogger, "insert2CDRMap: key="<<key<<", "<<caller<<"->"<<called<<", type="<<pMessage->getProtocolType()<<", status="<<pMessage->getStatus());
    }
    return 0;
}
    ...
    if (iter != m_cdrMap.end())
    {
        ...
        if(matchedMessage)
        {
            ...
            if((0 == strcmp(caller, matchedCaller)) && (0 == strcmp(called, matchedCalled)))
            {
                // duplicate call
                LoggerPtr rootLogger = Logger::getRootLogger();
                LOG4CXX_WARN(rootLogger, ++m_nRepeatDRs<<": same call, same key="<<key<<", diffStart="<<diffStart<<", diffEnd="<<diffEnd<<", "<<caller<<"->"<<called
                    <<", old(type="<<matchedMessage->getProtocolType()<<", id="<<matchedMessage->getCDRID()
                    <<", opc="<<matchedMessage->getOPC()<<", dpc="<<matchedMessage->getDPC()<<", packID="<<matchedMessage->getPackID()
                    <<"), new(type="<<pMessage->getProtocolType()<<", id="<<pMessage->getCDRID()
                    <<", opc="<<pMessage->getOPC()<<", dpc="<<pMessage->getDPC()<<", packID="<<pMessage->getPackID()<<")");
            }else{
                // different call
                LoggerPtr rootLogger = Logger::getRootLogger();
                LOG4CXX_ERROR(rootLogger, ++num<<": different call, same key="<<key<<", diffStart="<<diffStart<<", diffEnd="<<diffEnd
                    <<", old("<<matchedCaller<<"->"<<matchedCalled<<", id="<<matchedMessage->getCDRID()<<", type="<<matchedMessage->getProtocolType()
                    <<", opc="<<matchedMessage->getOPC()<<", dpc="<<matchedMessage->getDPC()<<", packID="<<matchedMessage->getPackID()
                    <<"), new("<<caller<<"->"<<called<<", id="<<pMessage->getCDRID()<<", type="<<pMessage->getProtocolType()
                    <<", opc="<<pMessage->getOPC()<<", dpc="<<pMessage->getDPC()<<", packID="<<pMessage->getPackID()<<")");
            }
            ret = 1;
        }
...
void ZxComm::checkCDR(){
          ...
          if(pMessage->ifSend()){
            ...
            if(IAM == status){
                if(now - cdrTime > m_msgDelayLimit){
                    if(pMessage->ifMatched()){
                        bDel = true;
                    }else{
                        if(now - cdrTime > m_beginEndInterval){
                            ++nTimeout;
                            bDel = true;
                            if(G::isTestPhone((char*)caller) || G::isTestPhone((char*)called)){
                                LoggerPtr rootLogger = Logger::getRootLogger();  
                                LOG4CXX_WARN(rootLogger, "checkCDR: dismatched IAM, "<<caller<<"->"<<called<<", type="<<type
                                   <<", CDRID="<<pMessage->getCDRID()<<", end="<<cdrTime<<", seqNum="<<seqNum<<", now="<<now);
                            }
                        }else{
                            bDel = false;
                        }
                ...
                bDel = true;
                LoggerPtr rootLogger = Logger::getRootLogger();
                LOG4CXX_ERROR(rootLogger, "wrong status="<<status<<", "<<caller<<"->"<<called
              ...
              if(now - cdrTime > m_msgDelayLimit){
                  ...
                  LOG4CXX_WARN(rootLogger, "timeout CDR: unsend, status="<<status<<", "<<caller<<"->"<<called
                    <<", type="<<type<<", seqNum="<<seqNum<<", now="<<now);                  
              }
          }
      }else{
          bDel = true;
  ...
  if(nTimeout>10000){
      LoggerPtr rootLogger = Logger::getRootLogger();
      LOG4CXX_WARN(rootLogger, nTimeout<<" dismatched IAM CDRs are timeout(>"<<m_beginEndInterval<<"s)!");
  }
  return;
}
