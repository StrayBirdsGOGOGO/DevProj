...
int SugwServer::sendMessage(CDRMessage* pMessage){
    ...
    caller = (char*)pMessage->getCaller();
    called = (char*)pMessage->getCalled();
    if(!G::isMobilePhone(caller) || !G::isMobilePhone(called)){
        return -1;
    }
    
    cdrTime = pMessage->getEndTime();
    seqNum = (char*)pMessage->getSeqNum();
    step = pMessage->getStatus();
    if((IAM != step) && (Closure != step)){
        LoggerPtr rootLogger = Logger::getRootLogger();  
        LOG4CXX_ERROR(rootLogger, "Error step="<<step<<", "<< caller<<"->"<<called);
        return -1;
    }
    ...
    time(&now);
	/*
    if((now-cdrTime) > ZxComm::m_msgDelayLimit){
        //timeout
        if((++nTimeout % 10000) == 0){
            LoggerPtr rootLogger = Logger::getRootLogger();
    ...
    */
    ...
    //send data format: 'S'+len+step+state+'|'+seqNum+'|'+caller+':'+called+'T'
    //min len = 36; max len = 79
    ...
    sendBuf[1]=0;
    sendBuf[2]=0x30 + step;
    sendBuf[3]=0x30 + state;
    sendBuf[4]='|'; 
    sendLen = 5;
    memcpy(sendBuf+sendLen,seqNum,strlen(seqNum));
    sendLen += strlen(seqNum);
    sendBuf[sendLen++]='|';     
    len = strlen(caller);
    memcpy(sendBuf+sendLen,caller,len);
    sendLen += len;
    sendBuf[sendLen++]=':';     
    len = strlen(called);
    memcpy(sendBuf+sendLen,called,len);
    sendLen += len;
    sendBuf[sendLen++]='T';
    sendBuf[sendLen]=0;
    sendBuf[1]=sendLen&0xff-1;
    
    if(sendLen<MIN_SEND_DATA_LEN || sendLen>MAX_SEND_DATA_LEN){
        LoggerPtr rootLogger = Logger::getRootLogger();  
        LOG4CXX_ERROR(rootLogger, "Error sendLen="<<sendLen<<", step="<<step<<", seqNum="<<seqNum<<", "<<state<<":"<< caller<<"->"<<called<<", buf="<<sendBuf);
        return -1;
    }
    
    //send data to sugw
    if(!gSockFdMap.empty()){
        ret = sktSend(sendBuf, sendLen);
        if(0 != ret){
    ...
    if(IAM == step){
        m_nSendStart++;
    }else if(Closure == step){
        m_nSendEnd++;
    }

    if(G::isTestPhone(caller) || G::isTestPhone(called)){
        LoggerPtr rootLogger = Logger::getRootLogger();  
        LOG4CXX_DEBUG(rootLogger, "sendToSugw: step="<<step<<", "<< caller<<"->"<<called<<", seqNum="<<seqNum<<", sendLen="<<sendLen);
    }
...
int SugwServer::sktSend(PUCHAR sendBuf, int sendLen) {
    ...
    case 0:  
        //timeout
        LOG4CXX_ERROR(rootLogger, "send msg to sugw timeout!");
        removeSigSockList();
        return -1;
    default:  
        break;
...
void sugwSendThread(void) {
  ...
  while (!G::m_bSysStop && rDataControl.isActive()) {
    ...
    if (pMessage){
        ret = SugwServer::sendMessage(pMessage);
